    ...
    namespace Talent.Services.Profile.Domain.Services
    {
        public class ProfileService : IProfileService
        {
            ...
            {
            IEnumerable<TalentSnapshotViewModel> users = null;
          //  _userRepository.Collection.AsEnumerable

            users = _userRepository.Collection.Where(user => user.IsDeleted == false).OrderBy(user => user.CreatedOn).Select(user =>
            ...
            public async Task<IEnumerable<TalentSnapshotViewModel>> GetTalentSnapshotList()
            {
                IEnumerable<TalentSnapshotViewModel> users = null;
                users = _userRepository.Collection.AsEnumerable().OrderByDescending(user => user.CreatedOn).Select(user =>
                ...
                new TalentSnapshotViewModel
                {
                    ...
                    GithubURL = user.LinkedAccounts.Github,
                    Visa = user.VisaStatus,
                    Position = (user.Experience.FirstOrDefault() == null) ? "" : user.Experience.FirstOrDefault().Position,
                    CurrentEmployment = (user.Experience.FirstOrDefault() == null)? "" : user.Experience.FirstOrDefault().Company,
                  Skills = user.Skills.Select(skill => skill.Skill).ToList()
            ...
            public async Task<IEnumerable<TalentSuggestionViewModel>> GetFullTalentList(int pageNumber, int pageSize)
            {

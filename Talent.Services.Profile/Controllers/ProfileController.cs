...
namespace Talent.Services.Profile.Controllers
{
    ...
    public class ProfileController : Controller
    {
        ...
        {
            Console.WriteLine("inside get talent profile");
            String talentId = String.IsNullOrWhiteSpace(id) ? _userAppContext.CurrentUserId : id;
            Console.WriteLine("talent id is "+ talentId);
        ...
        [Authorize(AuthenticationSchemes = JwtBearerDefaults.AuthenticationScheme, Roles = "recruiter, employer")]
        public async Task<IActionResult> GetTalentListAsync(int pageNumber = 1, int pageSize = 5)
        {
            Console.WriteLine("page no"+pageNumber+"  size"+pageSize);
            try
            {
                var result = await _profileService.GetTalentSnapshotList();
                var newResult = result.Skip((pageNumber - 1) * pageSize).Take(pageSize);

                return Json(new { Success = true, Data = newResult, count = result.Count() });
            }
